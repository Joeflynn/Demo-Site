...
export const Canvas: React.FC<DrawingCanvasProps> = ({
  ...
  const [, setIsPainting] = useState<boolean>(false)
  ...
  const [brushCanvas, setBrushCanvas] = useState<HTMLCanvasElement | null>(null)
  const [brushCtx, setBrushCtx] = useState<CanvasRenderingContext2D | null>(null)
  ...
  useEffect(() => {
    if (shouldUndo) {
      ...
      if (lastState) {
        ...
        redoStates.current.push(ctx.getImageData(0, 0, ctx.canvas.width, ctx.canvas.height))
  ...
  useEffect(() => {
    if (shouldRedo) {
      ...
      if (lastRedoState) {
        ...
        canvasStates.current.push(ctx.getImageData(0, 0, ctx.canvas.width, ctx.canvas.height))
  ...
  function setPixelColor(imageData: ImageData, x: number, y: number, color: number[]) {
    const index = (y * imageData.width + x) * 4
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (isFilling) {
      ...
      floodFill(ctx, Math.floor(x), Math.floor(y), brushColor, fillTolerance * 2.55) // Adjust tolerance as needed
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLCanvasElement>) => {
    if (isCreatingShape && shapeStartPoint) {
      ...
      const brushSize = brushWidth * (pressureSize ? pressure : 0.5) + randomSize
      ...
      if (isErasing) {
        ...
        console.log('DrawingCanvas.tsx: handlePointerMove: isErasing: ' + isErasing)
      } else {
        ctx.globalCompositeOperation = 'source-over'
      }
      ...
      if (isSmudging) {
        ...
        for (let i = 0; i < dist; i += stepSize) {
          ...
          const newCopy = ctx.getImageData(newCopyX, newCopyY, brushSize, brushSize)
          setCopy(newCopy)
        }
      } else {
        for (let i = 0; i < dist; i += stampSpacing) {
          ...
          let brushAngle = Math.atan2(currentPoint.y - lastPoint!.y, currentPoint.x - lastPoint!.x)

          brushAngle = brushAngle + brushRotation

          // eslint-disable-next-line @typescript-eslint/no-unused-expressions
          Math.random() * brushRotationJitter - brushRotationJitter / 2
          brushAngle += (Math.random() * (brushRotationJitter - brushRotationJitter / 2)) / 20
